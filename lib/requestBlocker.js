...
const {makeRequest} = require("../bladeui/js/utils/request");
...
function updateBladeData()
{
  browser.storage.sync.get(null, (data) =>
  {
    ...
    if (data.bladeUserData)
    {
      try
      {
        userBladeId = data.bladeUserData.userCode;
        token = data.bladeUserData.token;
      }
      catch (error)
      {
        console.error(error);
...
browser.storage.onChanged.addListener((changes, area) =>
{
  ...
  if (changedItems.includes("bladeUserData"))
  {
    updateBladeData();
  }
});
...
browser.webRequest.onBeforeRequest.addListener(details =>
{
  ...
  if (bladeUrl && details.url.includes(bladeUrl + "/www/delivery/lg_blade.php"))
  {
    makeRequest({
      method: "post",
      url: "/jwt/feed"
