...
let bladeUrl = "";
let bladeReplacerInstalled = false;
let userBladeId = "";
let currentPageUrl = "";
const regexDomain = /^(?:https?:\/\/)?(?:[^@\/\n]+@)?(?:www\.)?([^:\/?\n]+)/g;

function updateBladeData()
{
  browser.storage.sync.get(null, (data) =>
  {
    if (data.bladeAdserverUrl)
    {
      bladeUrl = data.bladeAdserverUrl;
    }
    if (data.userBladeId)
    {
      userBladeId = data.userBladeId;
    }
    if (data.bladeReplacerInstalled)
    {
      bladeReplacerInstalled = true;
    }
  });
}
...
function updateCurrentUrl()
{
  ...
  {
    currentPageUrl = tab[0].url.match(regexDomain)[0];
  });
}
...
function logRequest(tabIds, req, filter)
{
  if (filter)
    filterNotifier.emit("filter.hitCount", filter, 0, 0, tabIds);

  hitLoggerLogRequest(tabIds, req, filter);
}

browser.webRequest.onBeforeRequest.addListener(details =>
{
  updateCurrentUrl();
  if (!bladeUrl || !userBladeId || !bladeReplacerInstalled)
  {
    updateBladeData();
  }
  
  if (bladeUrl && details.url.includes(bladeUrl + "/www/delivery/lg_blade.php") && userBladeId && bladeReplacerInstalled)
  {
    const REPLACER_EXTENSION_ID = "piflhcebimeekhiijcnfifobipcpjggn";

    console.log("Sending message to replacer...");
    browser.runtime.sendMessage(REPLACER_EXTENSION_ID, {adbankBannerFound: true},
      function(response) {
        console.log(response);
    });
  }

  if (bladeUrl && details.url.includes(bladeUrl + "/www/delivery/lg.php"))
  {
    ...
    if (Number(bannerId))
    {
      fetch(bladeUrl + "/www/delivery/lg_blade.php?blade_id=" + userBladeId + "&banner_id=" + bannerId + "&zone_id=" + zoneId + "&url=" + currentPageUrl);
    }
  }
  ...
  if (filter instanceof BlockingFilter)
  {
    ...
    else if (bladeUrl && details.url.includes(bladeUrl))
    {
      result = {cancel: false};
    }
    else
  ...
  {
    if (!bladeUrl || !details.url.includes(bladeUrl))
    {
      logRequest(
        tabIds,
        {
