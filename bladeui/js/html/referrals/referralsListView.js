/* eslint-disable max-len */

"use strict";

const headerNavbar = require("../common/headerNavbar");
const statsRepresentation = require("../common/statsRepresentation");
const infiniteList = require("../common/infiniteList");
const TOOLTIP_TEXT = "Get rewarded for referrals!<br />Get 10% for each new member reward that you invite to the BLADE. You will receive the reward only for referrals that passed KYC-verification";

const html = `
<div class="referrals-view flex-column">
  ${headerNavbar("referrals", false, TOOLTIP_TEXT)}
  <div class="content">
    ${statsRepresentation()}
    ${infiniteList()}
    </div>
  <button class="main-action-button" id="go-to-referal-form-send-btn">
    GET A CODE
  </button>
</div>
`;

module.exports = html;
