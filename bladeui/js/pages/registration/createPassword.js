...
class CreatePassword extends BaseClass
{
  ...
  sendRequest()
  {
    makeRequest({
      method: "post",
      url: "/api/user",
      data: {password: this.PasswordHelper.password, fingerprint: this.fingerprint}
    })
    .then((response) =>
    {
      ...
      browser.storage.sync.set({
        bladeUserData: {
          ...
          referralCode: response.data.referral_link
        }
      });
      ...
      {
        setTimeout(() => browser.tabs.create({url: browser.extension.getURL("welcome.html?code=" + response.referred_by)}), 1000);
      }
